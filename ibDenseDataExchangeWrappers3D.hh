...
namespace plb {
  ...
  void setSpheresOnLattice(MultiBlockLattice3D<T,Descriptor> &lattice,
                           ...
                           bool initVelFlag)
  {
    ClearParticleData<T, Descriptor>* clearParticleData = new ClearParticleData<T, Descriptor>();
    applyProcessingFunctional(clearParticleData, lattice.getBoundingBox(), lattice);
                            ...
                            PhysUnits3D<T> const &units)
  {
    ...
    std::vector<T> force(nPart * 3, 0.0);
    std::vector<T> torque(nPart * 3, 0.0);
    std::vector<Array<T,3>> x_lb;
    x_lb.reserve(nPart);

    for(plint iPart = 0; iPart < nPart; iPart++){
        x_lb.push_back(Array<T,3>(
            units.getLbPosition(wrapper.lmp->atom->x[iPart][0]),
    ...
    for(plint iPart=0;iPart<nPart;iPart++){
      ...
      if(liggghts_ind < 0 || liggghts_ind >= wrapper.lmp->atom->nlocal + wrapper.lmp->atom->nghost) continue;
...
#endif /* IBDENSEDATAEXCHANGEWRAPPERS_HH_LBDEM */
