...
namespace plb{
  ...
  private:
    ...
    void setValues(typename IBdynamicsParticleData<T,Descriptor>::ParticleInfo &p,
                   T const sf, T const dx, T const dy, T const dz);
    void setToZero(typename IBdynamicsParticleData<T,Descriptor>::ParticleInfo &p);
  };
...
#include "ibDenseProcessors3D.hh"
