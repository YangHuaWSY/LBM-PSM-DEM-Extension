...
namespace MODEL_PARAMS
{
  ...
  // Wang cohesion model parameters
  static const char * A_L = "AL";

  // Wang DLVO model parameters
  static const char * EDL_z = "EDLz";
  static const char * Inv_Debye = "InvDebye";
  /* -----------------------------------------------------------------------
   * Utility functions
   * ----------------------------------------------------------------------- */
    ...
    for(int i=1; i < max_type+1; i++)
    {
      ...
      if(sanity_checks)
      {
        ...
        if(strcmp(lmp->update->unit_style,"cgs") == 0 && Yi < 5e7)
          lmp->error->all(FLERR,"youngsModulus >= 5e7 required for CGS units");
      }
  ...
  MatrixProperty* createAL(PropertyRegistry & registry, const char * caller, bool sanity_checks)
  {
    return createPerTypePairProperty(registry, A_L, caller);
  }

  MatrixProperty* createEDLz(PropertyRegistry & registry, const char * caller, bool sanity_checks)
  {
    return createPerTypePairProperty(registry, EDL_z, caller);
  }

  MatrixProperty* createInverseDebye(PropertyRegistry & registry, const char * caller, bool sanity_checks)
  {
    return createPerTypePairProperty(registry, Inv_Debye, caller);
  }
}
